...
if (passwordLogin) {
  // Add event listener only if passwordLogin exists
  passwordLogin.addEventListener("input", function () {
    const inputValue = this.value.trim();
    if (inputValue.length > 0) {
      eyeIconContainerLogin.classList.remove("d-none");
    } else {
      eyeIconContainerLogin.classList.add("d-none");
    }
  });

  // Check if eyeIconContainer exists before adding event listener
  if (eyeIconContainerLogin) {
    eyeIconContainerLogin.addEventListener("click", function () {
      const type =
        passwordLogin.getAttribute("type") === "password" ? "text" : "password";
      passwordLogin.setAttribute("type", type);
      // Store a reference to the <i> element separately
      const eyeIcon = eyeIconContainerLogin.querySelector("i");
