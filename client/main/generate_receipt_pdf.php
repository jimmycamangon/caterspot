...
$balance_paid = (float) $_GET['balance_paid'];
$balance = (float) $_GET['balance'];
...
// Invoice title
$pdf->SetFont('Times', 'B', 14); // Times New Roman, bold, size 16
$pdf->Cell(0, 10, 'Receipt for Transaction No. ' . $transactionNo, 0, 1, 'C');
$pdf->Ln(10); // Add some vertical space
...
if ($initial_payment > 0 || $balance > 0) {
    ...
    $pdf->Cell(40, 7, 'PHP ' . $tax, 0, 1, 'R');
    $pdf->SetFont('Times', 'B', 10); // Times New Roman, bold, size 12
    $pdf->Cell(150, 7, 'Grand Total:', 0, 0, 'R');
    $pdf->Cell(40, 7, 'PHP ' . $grandTotal, 0, 1, 'R');
    $pdf->SetFont('Times', '', 10); // Reset font style to regular
    $pdf->Cell(150, 7, '', 0, 0, 'R');
    $pdf->Cell(40, 7, '__________________________________', 0, 1, 'R');
    $pdf->Cell(150, 7, 'Initial Payment:', 0, 0, 'R');
    $pdf->Cell(40, 7, 'PHP ' . $initial_payment, 0, 1, 'R');
    $pdf->Cell(150, 7, 'Previous Balance:', 0, 0, 'R');
    $pdf->Cell(40, 7, 'PHP ' . $balance, 0, 1, 'R');
    $pdf->Cell(150, 7, 'Amount Tendered:', 0, 0, 'R');
    $pdf->Cell(40, 7, 'PHP ' . $balance_paid, 0, 1, 'R');
    $pdf->SetFont('Times', 'B', 10); // Times New Roman, bold, size 12
    $pdf->Cell(150, 7, 'New Balance:', 0, 0, 'R');
    $pdf->Cell(40, 7, 'PHP 0.00', 0, 1, 'R');

    $new_balance = $balance_paid - $balance;
    ...
    if ($new_balance > 0) {
        $pdf->Cell(150, 7, 'Change:', 0, 0, 'R');
        $pdf->Cell(40, 7, 'PHP ' . number_format($new_balance, 2), 0, 1, 'R'); // Format the new_balance variable
        $pdf->SetFont('Times', '', 10); // Reset font style to regular
    }
...
// Output the PDF
$pdf->Output('Receipt_' . $transactionNo . '.pdf', 'D');
?>